package com.example03;

import org.junit.jupiter.api.Test;

import static org.junit.jupiter.api.Assertions.assertEquals;

class StringUtils1Test {

    private static final String MAX_STRING = "";

    @Test
    void joinNullAndNull() {
        String result = StringUtils1.joinDescriptions(null, null);
        assertEquals("", result);
        assertEquals(0, result.length());
    }

    @Test
    void joinNullAndNormalString() {
        String result = StringUtils1.joinDescriptions(null, "AAABBB");
        assertEquals("AAABBB", result);
        assertEquals(6, result.length());
    }


    @Test
    void joinNormalStringAndNull() {
        String firstString = new String("LoremIpsum");
        String result = StringUtils1.joinDescriptions(firstString, null);
        assertEquals(firstString, result);
        assertEquals(firstString.length(), result.length());
    }

    @Test
    void joinNullAndLongString() {
        String result = StringUtils1.joinDescriptions(null, new String(MAX_STRING + "ABCDEF"));
        assertEquals(MAX_STRING, result);
        assertEquals(4000, result.length());
    }

    @Test
    void joinLongStringAndNull() {
        String result = StringUtils1.joinDescriptions(new String(MAX_STRING + "ABCDEF"), null);
        assertEquals(MAX_STRING, result);
        assertEquals(4000, result.length());
    }

    @Test
    void joinLongStringAndNormalString() {
        String result = StringUtils1.joinDescriptions(new String(MAX_STRING + "ABCDEF"), "XXXX");
        assertEquals(MAX_STRING, result);
        assertEquals(4000, result.length());
    }

    @Test
    void joinNormalStringAndLongString() {
        String normalString = new String("XXXX");
        String result = StringUtils1.joinDescriptions(normalString, new String(MAX_STRING + "ABCDEF"));
        assertEquals(new String(normalString + MAX_STRING.substring(0, MAX_STRING.length() - normalString.length())), result);
        assertEquals(4000, result.length());
    }

    @Test
    void joinMaxStringAndNull() {
        String result = StringUtils1.joinDescriptions(new String(MAX_STRING), null);
        assertEquals(MAX_STRING, result);
        assertEquals(4000, result.length());
    }

    @Test
    void joinNullAndMaxString() {
        String result = StringUtils1.joinDescriptions(null, new String(MAX_STRING));
        assertEquals(MAX_STRING, result);
        assertEquals(4000, result.length());
    }

    @Test
    void joinLongStringAndLongString() {
        String result = StringUtils1.joinDescriptions(new String((MAX_STRING) + "AAAA"), new String(MAX_STRING + "BBBB"));
        assertEquals(MAX_STRING, result);
        assertEquals(4000, result.length());
    }

    @Test
    void joinNormalStringAndNormalString() {
        String result = StringUtils1.joinDescriptions("12345678901234", "AAAAABBBBBCCCCC");
        assertEquals("12345678901234AAAAABBBBBCCCCC", result);
    }

}
