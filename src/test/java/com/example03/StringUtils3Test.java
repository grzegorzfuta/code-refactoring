package com.example03;

import org.junit.jupiter.params.ParameterizedTest;
import org.junit.jupiter.params.provider.Arguments;
import org.junit.jupiter.params.provider.MethodSource;

import java.util.stream.Stream;

import static org.junit.jupiter.api.Assertions.assertEquals;

class StringUtils3Test {

    private static final String MAX_STRING = "";
    private static final String NORMAL_STRING = "QX9jfOd0QwQwCHOz";
    private static final int MAX_DESCRIPTION_LENGTH = 4000;


    static Stream<Arguments> testArgumentProvider() {
        return Stream.of(
                Arguments.of(null, null, ""),

                Arguments.of(null, NORMAL_STRING, NORMAL_STRING),
                Arguments.of(NORMAL_STRING, null, NORMAL_STRING),

                Arguments.of(null, MAX_STRING + NORMAL_STRING, MAX_STRING),
                Arguments.of(MAX_STRING + NORMAL_STRING, null, MAX_STRING),

                Arguments.of(MAX_STRING + NORMAL_STRING, NORMAL_STRING, MAX_STRING),
                Arguments.of(NORMAL_STRING, MAX_STRING + NORMAL_STRING, (NORMAL_STRING + MAX_STRING).substring(0, MAX_DESCRIPTION_LENGTH)),

                Arguments.of(MAX_STRING, null, MAX_STRING),
                Arguments.of(null, MAX_STRING, MAX_STRING),

                Arguments.of(MAX_STRING, NORMAL_STRING, MAX_STRING),
                Arguments.of(NORMAL_STRING, MAX_STRING, (NORMAL_STRING + MAX_STRING).substring(0, MAX_DESCRIPTION_LENGTH)),

                Arguments.of(NORMAL_STRING, NORMAL_STRING, NORMAL_STRING + NORMAL_STRING)
        );
    }

    @ParameterizedTest
    @MethodSource("testArgumentProvider")
    void testJoinDescriptions(String description1, String description2, String expectedResult) {
        String actualResult = StringUtils3.joinDescriptions(description1, description2);
        assertEquals(expectedResult, actualResult);
    }

}
